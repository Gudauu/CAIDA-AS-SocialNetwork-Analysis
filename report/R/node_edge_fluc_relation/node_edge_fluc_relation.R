...
function_fluc_relation_across_years <- function(year_start, year_end){
    ...
    p <- ggplot(data = df, aes(x = years)) +
        # added ratio line
        geom_line(aes(y = added_ratio, color = "added:ASR with one end in added AS")) +
        geom_point(aes(y = added_ratio, color = "added:ASR with one end in added AS"), size = 2) +
        geom_text(aes(y = added_ratio, label = round(added_ratio, digits = 3)), vjust = -1, size = 2, color = "black") +
        # added ratio line
        geom_line(aes(y = added_ratio_both, color = "added:ASR with both ends in added AS")) +
        geom_point(aes(y = added_ratio_both, color = "added:ASR with both ends in added AS"), size = 2) +
        geom_text(aes(y = added_ratio_both, label = round(added_ratio_both, digits = 3)), vjust = -1, size = 2, color = "black") +
        # added standard line
        geom_line(aes(y = added_std, color = "added:pivot for one")) +
        geom_point(aes(y = added_std, color = "added:pivot for one"), size = 2) +
        geom_text(aes(y = added_std, label = round(added_std, digits = 3)), vjust = -1, size = 2, color = "black") +
        # added standard line(both)
        geom_line(aes(y = added_both_std, color = "added:pivot for both")) +
        geom_point(aes(y = added_both_std, color = "added:pivot for both"), size = 2) +
        geom_text(aes(y = added_both_std, label = round(added_both_std, digits = 3)), vjust = -1, size = 2, color = "black") +
        # deleted ratio line
        geom_line(aes(y = deleted_ratio, color = "deleted:ASR with one end in deleted AS")) +
        geom_point(aes(y = deleted_ratio, color = "deleted:ASR with one end in deleted AS"), size = 2) +
        geom_text(aes(y = deleted_ratio, label = round(deleted_ratio, digits = 3)), vjust = -1, size = 2, color = "black") +
        # deleted ratio line(both)
        geom_line(aes(y = deleted_ratio_both, color = "deleted:ASR with both ends in deleted AS")) +
        geom_point(aes(y = deleted_ratio_both, color = "deleted:ASR with both ends in deleted AS"), size = 2) +
        ...
        geom_line(aes(y = deleted_std, color = "deleted:pivot for one")) +
        geom_point(aes(y = deleted_std, color = "deleted:pivot for one"), size = 2) +
        geom_text(aes(y = deleted_std, label = round(deleted_std, digits = 3)), vjust = -1, size = 2, color = "black") +
        # deleted standard line
        geom_line(aes(y = deleted_both_std, color = "deleted:pivot for both")) +
        geom_point(aes(y = deleted_both_std, color = "deleted:pivot for both"), size = 2) +
...
function_fluc_relation_across_years(2000, 2020)
